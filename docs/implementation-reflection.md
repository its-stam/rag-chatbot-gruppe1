...
- **Arbeitsteilung genutzt.** Supabase-Zugang (Teammitglied A) und n8n-Zugang (Rustam) sauber getrennt abgearbeitet.
...
**Fix:** Symlink entfernt, die 5 `.md` als echte Kopie direkt unter `~/.n8n-files/company-docs/hr-set-a/` abgelegt (ausserhalb der macOS-geschützten Ordner).
